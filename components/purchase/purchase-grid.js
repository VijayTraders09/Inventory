...
export default function PurchaseGrid() {
  ...
  const fetchPurchases = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.error || "Failed to fetch purchases");
      }
    } catch (error) {
      toast.error(
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `purchases_${new Date().toISOString().split("T")[0]}.xlsx`;
  ...
  // Custom SearchableSelect component for products that uses local data
  const ProductSearchableSelect = ({
    ...
    return (
      <div className="relative">
        ...
        {isOpen && !disabled && (
          <div className="absolute z-10 bottom-0 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
            <div className="p-2">
              <input
                type="text"
  ...
  const handleEdit = (purchase) => {
    ...
    setFormData({
      ...
      modeOfTransport: purchase.modeOfTransport,
      remark: purchase.remark || "",
    });

    // Populate stock entries from the purchase
    const entries = purchase.stockEntries.map((entry) => ({
      _id: entry._id,
  ...
  const handlePrint = (purchase) => {
    setSelectedPurchasePrint(purchase);
  ...
  const handleDeletePurchase = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.error || "Failed to delete purchase");
      }
    } catch (error) {
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage your purchase records</p>
          </div>
          <div className="flex space-x-2">
            <Button
          ...
          >
            <AgGridReact
              ...
              overlayLoadingTemplate={
                '<div class="ag-overlay-loading-center">Loading purchases...</div>'
              }
              overlayNoRowsTemplate={
                '<div class="ag-overlay-loading-center">No purchases found</div>'
              }
            />
          </div>
        </div>
