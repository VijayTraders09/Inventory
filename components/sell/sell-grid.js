  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.error || "Failed to fetch sales");
      }
    } catch (error) {
      toast.error(
      ...
      <div className="space-y-6 p-6">
        ...
        <form onSubmit={handleSearch} className="flex gap-2">
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              placeholder="Search sales by invoice..."
              value={localSearch}
          ...
          >
            <AgGridReact
              ...
              overlayLoadingTemplate={
                '<div class="ag-overlay-loading-center">Loading sales...</div>'
              }
              overlayNoRowsTemplate={
                '<div class="ag-overlay-loading-center">No sales found</div>'
              }
            />
          </div>
        </div>
        ...
        <Dialog open={showBillPreview} onOpenChange={setShowBillPreview}>
          <DialogContent className="sm:max-w-[400px] max-h-screen overflow-auto">
            <DialogHeader>
              <DialogTitle>Sales Bill Preview</DialogTitle>
            </DialogHeader>
            {selectedSalePrint && (
              <div className="space-y-4">
                <div
                ...
                >
                  <div className="text-center mb-4">
                    <div className="font-bold text-lg mb-2">SALES BILL</div>
                  ...
                  <div className="text-center">
                    ...
                    {selectedSalePrint.remark && (
                      <div className="mt-2">
                        Remark: {selectedSalePrint.remark}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="print-container">
          ...
          <div className="bill-content">
            ...
            <table className="bill-table">
              ...
              <tbody>
                {selectedSalePrint.stockEntries.map((entry, index) => (
                  <tr key={index}>
                    <td>{entry.productId.productName || entry.productId}</td>
                    <td>{entry.quantity}</td>
                    <td>{entry.godownId.godownName || entry.godownId}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="bill-footer">
              ...
              <div>Thank you for your business!</div>
              {selectedSalePrint.remark && (
                <div>Remark: {selectedSalePrint.remark}</div>
              )}
            </div>
          </div>
        </div>
